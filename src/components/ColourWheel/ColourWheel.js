...
class ColourWheel extends React.Component {
  constructor(props) {
    ...
    // NOTE: effectiveRadius will take into account padding as lineWidth.
    ...
    this.innerWheelBounds = calculateBounds(this.innerWheelRadius - lineWidth,
      this.innerWheelRadius);
    this.centerCircleBounds = calculateBounds(0, this.centerCircleRadius);
    this.firstSpacerBounds = calculateBounds(this.firstSpacerRadius - padding,
      this.firstSpacerRadius);
    this.secondSpacerBounds = calculateBounds(this.secondSpacerRadius - padding,
  ...
  getRelativeMousePos = (clientX, clientY) => {
    ...
    // This returns an object in which we have both mouse-pos relative to the canvas,
    // as well as the true-middle.
    return {
      fromCenter,
      onCanvas,
    };
  ...
  outerWheelClicked = (evtPos) => {
    ...
    // TODO: Let user set different return values in props; e.g. rbg obj, string, etc.
  ...
  drawOuterWheel = () => {
    ...
    // This value ensures that the stroke accounts for the lineWidth provided to produce
    // an accurately represented radius.
  ...
  drawInnerWheel = (animationPercentage = 0) => {
    ...
    const animateShades = () => {
      rgbShades.forEach((rgb, i) => {
        ...
        this.ctx.arc(width / 2, height / 2, effectiveRadius, startAngle, endAngle);
        this.ctx.lineWidth = lineWidth * animationPercentage; // Width of the innerWheel.
